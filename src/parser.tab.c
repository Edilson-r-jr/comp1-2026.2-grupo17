...
/* Marca que a linha atual teve erro semântico, para não imprimir resultado */
static int erro_semantico = 0;
...
void yyerror(const char *s) {
    /* Remove o prefixo em inglês e mantém o detalhe ("unexpected ..., expecting ...") */
    const char *prefixo = "syntax error, ";
    size_t n = strlen(prefixo);
    if (strncmp(s, prefixo, n) == 0) {
        s += n;
    }
    /* yylloc = posição do token que causou o erro */
